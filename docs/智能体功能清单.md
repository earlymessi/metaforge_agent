...
| **Plan** | 生成 Tool 步骤 | `BaseAgent.build_plan` → `llm_plan` | GLM `plan`（`LLM_PLAN_AGENTS`）；失败 → 各 Agent `build_rule_plan`；**events** 走 `events_collab` 固定阶段（无 LLM plan） |
...
| `LLM_PLAN_AGENTS` | 逗号分隔，默认 `scheduling`（events 已固定编排不在此列）；亦可含 `kitting,commitment,whatif` |
...
| 项 | 内容 |
|----|------|
| **智能体** | `events`（`EventsCollabBridge` → `events_collab`） |
| **路由** | GLM Router → `reschedule`；Guard 强制动态事件进 events；回退含「重排/故障/插单」 |
| **Plan** | **固定阶段编排**（无 LLM plan）；插单多轮补全走 `merge_insert_job` 链；产出 `events_trace` |
...
| 业务 Agent | `src/metaforge/agents/{scheduling,events_collab_bridge,…}.py`；events 编排 `events_collab/` |
